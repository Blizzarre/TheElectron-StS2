...
public static class ElectronMetrics
{
    private static readonly RunMetricsUploader<RunMetrics> Uploader = new(
        new MetricsUploaderConfig
        {
            ModName = "The Electron",
            EndpointUrl = "https://cyokwozapjzlccsanzwa.supabase.co/rest/v1/runs_electron",
            ApiKey = BuildSecrets.MetricsApiKey,
            ModVersionProvider = TheElectronMod.GetVersion,
            IsOwnCharacter = e => e is Character.TheElectron,
            AllowedAssemblies = new HashSet<Assembly>
            {
                typeof(Character.TheElectron).Assembly,
                typeof(CharacterModel).Assembly
            },
            Logger = TheElectronMod.Logger,
